<template>
  <div class="info">
      ...
      <div class="top">
        <div class="top-item">
          <div class="top-item-sub">
            <suc-input v-model="tableParams.dividerid" placeholder="设施标识码" :config="{ clearable: true }" style="width:200px"></suc-input>
          </div>
          <div class="top-item-sub">
    ...
    <jlss-add-modal :parentMydatasource="parentMydatasource" :title="modalTitle" :modal="modal" :formParams="formParams" @toggle="toggle" @on-submit="oncreate" >
    </jlss-add-modal>
...
import { JlssAddModal,DeleteModal } from "./modal";
...
export default class GivePlan extends Vue {
  ...
  async onpage() {
    ...
    let url = "server/dataCenter/divider/infoList";
    this.tableParams.dividerid = this.getTrim(this.tableParams.dividerid); //去除空格
  ...
  setCreate() {
    this.loading = true;
    let url = "server/dataCenter/divider/addInfo";
    console.log(this.formParams, "1111");
  ...
  async setEdit() {
    this.loading = true;
    let url = "server/dataCenter/divider/editInfo";
  ...
  onedit(row: any) {
    ...
    this.modalTitle = "编辑";
    this.modal = true;
    let dividerid = row.dividerid;
    this.editInfo(dividerid);
  }
  async editInfo(dividerid: any) {
    let url = "server/dataCenter/divider/getDetail";
    let data = await publicApi.getWithParam(url, { dividerid });
    this.formParams = data.data;
  }
  ondelete(row: any) {
    this.delmodel = true;
    this.delParams = {
      dividerid: row.dividerid,
    };
  }
  ...
  async save() {
    ...
    let url = "server/dataCenter/divider/delInfo";
