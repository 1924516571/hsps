...
*/
export async function toViewCenter(mapId, pos, yPixel) {
    const map = await olData.getMap(mapId);
    const view = map.getView();
    const center2 = view.getCenter();
    const size = map.getSize();
    if(center2&&size) {
        const centerPixel2 = map.getPixelFromCoordinate(center2); // 当前中心点的像素坐标
        const coords = coordinateTransform(pos, 'EPSG:4326',view.getProjection().getCode());
        const pointPixel = map.getPixelFromCoordinate(coords); // 当前点位的像素坐标
        const pointPixel2 = [size[0]/2, yPixel]; // 平移后点位的像素坐标
        const diffX = pointPixel2[0]-pointPixel[0],
            diffY = pointPixel2[1]-pointPixel[1];
        const centerPixel = [centerPixel2[0]-diffX, centerPixel2[1]-diffY]; // 平移后中心点的像素坐标
        const center = map.getCoordinateFromPixel(centerPixel);
        view.animate({center});
    }
}
...
export function defineProj(name, code) {
    ...
    **  定义4490坐标系 Proj4这个库使用EPSG.io上的坐标代码有bug，改用WKT的数据
...
/**
 *  @description: 已知当前视图下一个经纬度点位和像素距离数组，求另一个点位
 *  @param {Array} LonLat
 *  @param {Array} pixel
 *  @return {Array} LonLat
 *  @author xrx
 *  @date 2019/6/5 14:36
 */
export async function getLonLatFromPixelDis(mapId, LonLat, pixel) {
    let map = await olData.getMap(mapId);
 ...
 */
export function calcRadiusAccurate(coords, distance, viewProjection) {
    const earthRadius = 6371393;
    const diffA = distance/(earthRadius*Math.cos(coords[1]/180*Math.PI));
    const diffDeg = diffA/Math.PI*180;
    if(viewProjection==='EPSG:4326') {
        return diffDeg;
    }
    const coords2 = [coords[0]+diffDeg, coords[1]];
    const aCoords = coordinateTransform(coords, 'EPSG:4326', viewProjection),
        aCoords2 = coordinateTransform(coords2, 'EPSG:4326', viewProjection);
    return aCoords2[0] - aCoords[0];
}
...
export function getGeodesicDistance(projection, point1, point2) {
    ...
    let s = Sphere.getDistance(point11, point21);
    return s;
}
