...
class EchartsLayer {
    ...
    _createLayerContainer() {
        ...
        const viewEle = this._map.getViewport();
        const {children} = viewEle;
        const {length} = children;
        let hasInsert = false;
        for(let i=0;i<length;i++) {
            const ele = children.item(i);
            if(ele.className==='ol-overlaycontainer') {
                viewEle.insertBefore(this._container, ele);
                hasInsert = true;
                break;
            }
        }
        if (!hasInsert) {
            viewEle.appendChild(this._container);
        }
    };
